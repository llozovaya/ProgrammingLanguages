...
fun what_month(day: int) =
  let val month_len = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
  in
      1 + number_before_reaching_sum(day, month_len)
  end
...
fun oldest(dates: (int*int*int) list) =
  if null(dates) then NONE
      ...
      let
	  fun is_oldest( date: int*int*int, dates2: (int*int*int) list) =
	    if null dates2 then date
	    else
		if is_older(date, hd dates2) then is_oldest(date, tl dates2)
		else
		    is_oldest(hd dates2, tl dates2)
      in
	  SOME(is_oldest(hd dates, tl dates))
      end
