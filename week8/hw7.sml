...
fun shift (e,deltaX,deltaY) =
    case e of
        NoPoints => e
      | Point (x,y) => Point (x+deltaX, y+deltaY)
      | Line (k,b) => Line (k, b + deltaY - k*deltaX)
...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift (x,y,e) => shift(eval_prog(e,env),x,y)
...
fun preprocess_prog exp =
    case exp of
        LineSegment (xStart,yStart,xEnd,yEnd) => 
            let 
              val sameX = real_close (xStart, xEnd)
              val leftToRight = xStart > xEnd
              val upToDown = yStart > yEnd
            in
              if (sameX andalso real_close (yStart, yEnd))
                then Point(xStart,yStart)
                else if (leftToRight orelse (sameX andalso upToDown)) 
                    then LineSegment (xEnd, yEnd, xStart, yStart)
                    else exp
            end
      | Intersect (e1,e2) => Intersect (preprocess_prog e1, preprocess_prog e2)
      | Let (v,e1,e2) => Let (v, preprocess_prog e1, preprocess_prog e2)
